...
| F3 | B | **Production could boot silently misconfigured.** Every setting fell back to a dev default, so a missing variable would have shipped `http://localhost:3000` unsubscribe links, `reply.mock.local` reply addresses, and CORS pinned to localhost | `assertProductionConfig()` logs every problem by variable name at boot, never logging values. It originally refused to start; that was reverted after a single mistyped variable took down health checks and public content along with the misconfigured feature. `STRICT_CONFIG_CHECK=true` restores hard-fail for staging |
