...
export function MessageBubble({ message, conversationId, onChanged }: { message: MessageView; conversationId: string; onChanged: () => void }) {
  ...
  return (
    <div className={`flex ${inbound ? "justify-start" : "justify-end"}`}>
      <div className={`max-w-[42rem] rounded-2xl border px-4 py-3 shadow-card ${inbound ? "border-sage bg-white" : "border-brand-200 bg-brand-50"}`}>
        ...
        {message.subject ? <p className="mb-1 text-sm font-semibold text-umber">{message.subject}</p> : null}

        {showHtml && message.htmlBody ? (
